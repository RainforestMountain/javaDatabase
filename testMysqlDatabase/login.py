...
class Login:
    def __init__(self):
        ...
        # 创建一个登录系统的按钮
        Button(self.login_root, command=lambda: self.login(account, password), text="登录", width=10).place(x=155,
                                                                                                            y=120)
        # 创建一个注册系统的按钮
        Button(self.login_root, command=lambda: self.register(account, password), text="注册", width=10).place(x=250,
                                                                                                               y=120)
    ...
    def login(self, account, password):
        ...
        if account.strip() and password.strip():
            sql = f"select * from stu_user where stu_account='{account}' and stu_password='{password}'"
    ...
    def register(self, account, password):
        ...
        # 判断信息是否输入完整
        if account.strip() and password.strip():
            sql = f"select * from stu_user where stu_account='{account}'"
            cursor = self.db.cursor
            cursor.execute(sql)
            results = cursor.fetchall()
            if len(results) == 0:
                sql = f"insert into stu_user values ('{account}', '{password}')"
                cursor.execute(sql)
                showinfo("提示", "注册成功")
            else:
